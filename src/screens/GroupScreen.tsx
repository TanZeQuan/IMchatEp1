...
type GroupScreenNavProp = NativeStackNavigationProp<MainStackParamList, "GroupScreen">;
...
export default function GroupScreen() {
  const navigation = useNavigation<GroupScreenNavProp>();
  const route = useRoute();
  const params = route.params as RouteParams;

  const groupName = params?.groupName || "群聊";
  ...
  const sendVoiceMessage = async (uri: string) => {
    try {
      ...
      console.log("Failed to send voice message from GroupScreen.");
    }
  };
